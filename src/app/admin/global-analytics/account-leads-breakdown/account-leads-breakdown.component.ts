...
export class AccountLeadsBreakdownComponent implements OnInit, OnDestroy {
  ...
  // ✅ NEW — holds plan/status/billingCycle/date filters passed from global analytics
  activeFilters: any = {};
  ...
  ngOnInit(): void {
    ...
    // ── Read sessionStorage FIRST — metric + loanType + filters ──
    const stored = sessionStorage.getItem('breakdownParams');
    if (stored) {
      try {
        const p          = JSON.parse(stored);
        ...
        this.loanType    = p.loanType    || 'all';
        this.accountName = p.accountName || '';
        this.activeFilters = p.filters  || {};   // ✅ restore page-level filters
      } catch {}
      sessionStorage.removeItem('breakdownParams');
    }

    // ── Unlock table AFTER everything is set ──
    this.tableReady = true;
    ...
    this.searchSubject.pipe(
      ...
      // this.onLazyLoad({ first: 0, rows: 10 });
      this.onLazyLoad({ first: this.leadsTable?.first || 0, rows: this.leadsTable?.rows || 10 });
    });
  }
  ...
  onLazyLoad(event: any): void {
    ...
    // ── Bankers metric → getBankWiseAnalytics ──
    if (this.isBankersMetric) {
      this.bankLoading = true;
      const from = event.first ?? 0;
      const rows = event.rows  ?? 10;
    ...
    // ✅ Pass activeFilters to the service call
    this.leadsService.getAccountLeadsBreakdown(
      this.accountId,
      this.metric,
      this.loanType,
  ...
  viewLead(leadId: any): void {
    if (['leads', 'files', 'credit', 'logins', 'inhouseRejects'].includes(this.metric)) {
      const lt = this.loanType.toLowerCase();
      if (lt === 'all' || lt === 'businessloan' || lt === 'business') {
        this.routingService.handleRoute('leads/view/' + leadId, null);
      } else {
        this.routingService.handleRoute('loan-leads/view/' + leadId, null);
      }
  ...
  // ── Helpers ──
  get metricLabel(): string  { return this.metricLabelMap[this.metric]  || this.metric;   }
  get metricIcon():  string  { return this.metricIconMap[this.metric]   || 'pi-list';      }
  get metricColor(): string  { return this.metricColorMap[this.metric]  || '#534AB7';      }
  get metricBg():    string  { return this.metricBgMap[this.metric]     || '#EEEDFE';      }

  get isAmountMetric(): boolean  { return this.metric === 'sanctioned' || this.metric === 'disbursed'; }
  get isBankersMetric(): boolean { return this.metric === 'bankers'; }
  ...
  getInternalStatusLabel(status: string): string {
    const map: Record<string, string> = {
      '1': 'Lead', '3': 'File', '5': 'Credit',
      '11': 'Login', '10': 'Inhouse Reject', 'rejected': 'Rejected',
    };
    return map[status] || status;
  }
  ...
  getStatusChipClass(status: string): string {
    const map: Record<string, string> = {
      '1': 'chip-lead', '3': 'chip-file', '5': 'chip-credit',
      '11': 'chip-login', '10': 'chip-reject', 'rejected': 'chip-reject',
    };
    return map[status] || '';
  }
